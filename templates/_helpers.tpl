...
{{- if .Values.externalS3.existingSecret -}}
{{- .Values.externalS3.existingSecretAccessSecretKey | default "access-secret" -}}
{{- else -}}
s3-access-secret
{{- end -}}
{{- end }}
...
{{- end -}}
{{- if and (include "n8n.needsRedis" .) (not .Values.externalRedis.host) -}}
{{- fail "Workers and webhook processors require Redis. Set externalRedis.host" -}}
{{- end -}}
...
{{- fail "persistence.enabled=true is not supported with workers. Use externalS3 for binary data persistence" -}}
{{- end -}}
{{- if and .Values.externalS3.enabled (not .Values.externalS3.host) -}}
{{- fail "externalS3.host is required when externalS3.enabled=true" -}}
{{- end -}}
{{- if and .Values.externalS3.enabled (not .Values.externalS3.bucketName) -}}
{{- fail "externalS3.bucketName is required when externalS3.enabled=true" -}}
{{- end -}}
{{- if and .Values.externalS3.enabled (not .Values.externalS3.bucketRegion) -}}
{{- fail "externalS3.bucketRegion is required when externalS3.enabled=true" -}}
{{- end -}}
{{- if and .Values.externalS3.enabled (not .Values.externalS3.existingSecret) (or (not .Values.externalS3.accessKey) (not .Values.externalS3.accessSecret)) -}}
{{- fail "externalS3.accessKey and externalS3.accessSecret (or externalS3.existingSecret) are required when externalS3.enabled=true" -}}
{{- end -}}
{{- if and .Values.externalS3.enabled .Values.externalS3.accessKey .Values.externalS3.existingSecret -}}
{{- fail "externalS3.accessKey/accessSecret and externalS3.existingSecret are mutually exclusive. Use one or the other" -}}
{{- end -}}
...
- name: N8N_EXTERNAL_STORAGE_S3_HOST
  value: {{ .Values.externalS3.host | quote }}
...
- name: N8N_EXTERNAL_STORAGE_S3_BUCKET_REGION
  value: {{ .Values.externalS3.bucketRegion | quote }}
